...
import com.fxiaoke.dataplatform.util.exception.MLBasicException
 ...
 *
  ...
  def daemonThread(runnable: Runnable): Thread =
    newThread(runnable, daemon = true)
 ...
 *
  ...
  def daemonThread(name: String, runnable: Runnable): Thread = 
    newThread(name, runnable, daemon = true)
 ...
 *
   ...
   * @throws MLBasicException If the file create fails
  ...
  def createFile(path: String): File = {
    ...
    if(!created)
      throw new MLBasicException("Failed to create file %s.".format(path))
    f
  }
